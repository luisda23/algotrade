...
const loginLimiter = rateLimit({
  windowMs: 5 * 60 * 1000,
  max: 15, // 15 intentos / 5 min / IP
  standardHeaders: true,
  legacyHeaders: false,
  message: errResp(
    RC.RL_LOGIN,
    "Too many login attempts. Wait 5 minutes before trying again.",
  ),
});
...
const signupLimiter = rateLimit({
  ...
  message: errResp(
    RC.RL_SIGNUP,
    "Too many accounts created from this IP. Wait one hour.",
  ),
});
...
function validatePassword(pw: string): PwError | null {
  if (typeof pw !== "string")
    return { code: RC.AUTH_PASSWORD_REQUIRED, fallback: "Password required" };
  if (pw.length < 8)
    return {
      code: RC.AUTH_PASSWORD_TOO_SHORT,
      fallback: "Password must be at least 8 characters",
    };
  if (pw.length > 128)
    return {
      code: RC.AUTH_PASSWORD_TOO_LONG,
      fallback: "Password is too long",
    };
  if (!/[A-Z]/.test(pw))
    return {
      code: RC.AUTH_PASSWORD_NEEDS_UPPER,
      fallback: "Password must contain at least one uppercase letter",
    };
  if (!/[a-z]/.test(pw))
    return {
      code: RC.AUTH_PASSWORD_NEEDS_LOWER,
      fallback: "Password must contain at least one lowercase letter",
    };
  if (!/\d/.test(pw))
    return {
      code: RC.AUTH_PASSWORD_NEEDS_DIGIT,
      fallback: "Password must contain at least one number",
    };
  return null;
}
...
router.post("/signup", signupLimiter, async (req: Request, res: Response) => {
  ...
  const SAFE_OK = okResp(
    RC.AUTH_SIGNUP_OK,
    "Account created. Sign in to verify your email.",
    { requiresLogin: true },
  );
  ...
  try {
    ...
    if (!email || !password || !name) {
      return res
        ...
        .json(
          errResp(
            RC.AUTH_EMAIL_PASSWORD_NAME_REQUIRED,
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return res
        .status(400)
        .json(errResp(RC.AUTH_EMAIL_INVALID, "Invalid email"));
    }
    if (email.length > 254) {
      return res
        .status(400)
        .json(errResp(RC.AUTH_EMAIL_TOO_LONG, "Email too long"));
    }
    const pwError = validatePassword(password);
    if (pwError)
      return res.status(400).json(errResp(pwError.code, pwError.fallback));
    if (name.length < 2 || name.length > 80) {
      return res
        .status(400)
        .json(errResp(RC.AUTH_NAME_INVALID, "Invalid name (2-80 characters)"));
    }
  ...
  try {
    ...
    if (!email || !password) {
      return res
        ...
        .json(
          errResp(
            RC.AUTH_EMAIL_PASSWORD_REQUIRED,
    ...
    if (!user) {
      return res
        .status(401)
        .json(errResp(RC.AUTH_INVALID_CREDENTIALS, "Invalid credentials"));
    }
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (typeof raw.name === "string") {
        ...
        if (name.length < 2 || name.length > 80) {
          return res
            .status(400)
            .json(
              errResp(RC.AUTH_NAME_INVALID, "Invalid name (2-80 characters)"),
            );
        }
        ...
        if (newEmail === me.email) {
          ...
          if (existing && existing.id !== req.userId) {
            return res
              .status(409)
              .json(
                errResp(RC.AUTH_EMAIL_IN_USE, "That email is already in use"),
              );
          }
          ...
          try {
            ...
            return res
              ...
              .json(
                errResp(
                  RC.AUTH_CODE_SEND_FAIL,
                  "Could not send the code to the new email",
                ),
              );
          }
      ...
      if (!nameUpdated && !emailChangeRequested) {
        return res
          .status(400)
          .json(errResp(RC.AUTH_NOTHING_TO_UPDATE, "Nothing to update"));
      }
      ...
      res.json({
        ...okResp(
          emailChangeRequested
            ? RC.AUTH_EMAIL_CHANGE_REQUESTED
            : RC.AUTH_PROFILE_UPDATED,
          emailChangeRequested
            ? "We sent a code to your new email"
            : "Profile updated",
        ),
        user,
        emailChangeRequested,
    ...
    try {
      ...
      if (!/^\d{6}$/.test(code)) {
        return res
          .status(400)
          .json(
            errResp(RC.AUTH_CODE_INVALID_FORMAT, "Invalid code (6 digits)"),
          );
      }
      ...
      if (
        ...
        return res
          .status(400)
          .json(
            errResp(RC.AUTH_NO_PENDING_EMAIL_CHANGE, "No pending email change"),
          );
      }
      if (user.verificationCodeExpires.getTime() < Date.now()) {
        return res
          ...
          .json(
            errResp(
              RC.AUTH_CODE_EXPIRED,
              "The code has expired. Request the change again.",
            ),
          );
      }
      if ((user.verificationAttempts || 0) >= 5) {
        return res
          ...
          .json(
            errResp(
              RC.AUTH_CODE_TOO_MANY_ATTEMPTS,
              "Too many attempts. Request the change again.",
            ),
          );
      }
      ...
      if (!match) {
        ...
        return res
          .status(401)
          .json(errResp(RC.AUTH_CODE_INCORRECT, "Incorrect code"));
      }
      ...
      if (taken && taken.id !== user.id) {
        ...
        return res
          ...
          .json(
            errResp(
              RC.AUTH_EMAIL_TAKEN_DURING_CHANGE,
              "That email was registered while you waited. Try another.",
            ),
          );
      }
      ...
      res.json({
        ...okResp(RC.AUTH_EMAIL_UPDATED, "Email updated"),
        user: updated,
        token,
      });
    } catch (error: any) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const pendingToken =
        typeof req.body?.pendingToken === "string" ? req.body.pendingToken : "";
      const code =
        typeof req.body?.code === "string" ? req.body.code.trim() : "";
      ...
      if (!pendingToken)
        return res
          .status(400)
          .json(
            errResp(RC.AUTH_LOGIN_PENDING_REQUIRED, "Login token required"),
          );
      if (!/^\d{6}$/.test(code)) {
        return res
          .status(400)
      ...
      try {
        ...
        return res
          ...
          .json(
            errResp(
              RC.AUTH_LOGIN_PENDING_EXPIRED,
              "Login attempt has expired. Sign in again.",
            ),
          );
      }
      if (payload?.type !== "pending-login" || !payload.userId) {
        return res
          .status(401)
          .json(errResp(RC.AUTH_LOGIN_PENDING_INVALID, "Invalid login token"));
      }
      ...
      if (!user.verificationCode || !user.verificationCodeExpires) {
        return res
          .status(400)
          .json(
            errResp(RC.AUTH_NO_PENDING_CODE, "No pending code. Sign in again."),
          );
      }
      if (user.verificationCodeExpires.getTime() < Date.now()) {
        return res
          ...
          .json(
            errResp(
              RC.AUTH_CODE_EXPIRED,
              "The code has expired. Sign in again.",
            ),
          );
      }
      if ((user.verificationAttempts || 0) >= 5) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (user.verificationCodeIssuedAt) {
        const sinceIssued =
          Date.now() - user.verificationCodeIssuedAt.getTime();
        if (sinceIssued < 60 * 1000) {
          return res
            .status(429)
            .json(
              errResp(
                RC.AUTH_CODE_RESEND_COOLDOWN,
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        return res
          .status(500)
          .json(errResp(RC.AUTH_RESET_EMAIL_FAIL, "Could not send the email"));
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!userId || !token) {
        return res
          .status(400)
          .json(errResp(RC.AUTH_RESET_LINK_INVALID, "Invalid link"));
      }
      ...
      if (!/[A-Z]/.test(newPassword) || !/\d/.test(newPassword)) {
        return res
          ...
          .json(
            errResp(
              RC.AUTH_PASSWORD_NEEDS_UPPER_DIGIT,
      ...
      if (!user || !user.resetToken || !user.resetTokenExpires) {
        return res
          ...
          .json(
            errResp(
              RC.AUTH_RESET_LINK_INVALID,
              "Link invalid or already used. Request a new one.",
            ),
          );
      }
    ...
    try {
      ...
      if (!/[A-Z]/.test(newPassword) || !/\d/.test(newPassword)) {
        return res
          ...
          .json(
            errResp(
              RC.AUTH_PASSWORD_NEEDS_UPPER_DIGIT,
              "New password must contain an uppercase letter and a number",
            ),
          );
      }
      if (currentPassword === newPassword) {
        return res
          ...
          .json(
            errResp(
              RC.AUTH_PASSWORD_SAME_AS_CURRENT,
      ...
      if (!match)
        return res
          ...
          .json(
            errResp(
              RC.AUTH_CURRENT_PASSWORD_WRONG,
      ...
      res.json({
        ...okResp(RC.AUTH_PASSWORD_UPDATED, "Password updated"),
        token,
      });
    } catch (error: any) {
      console.error(error);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.json(
        okResp(RC.AUTH_LOGOUT_EVERYWHERE_OK, "Signed out from all devices"),
      );
    } catch (error: any) {
      console.error(error);
      res.status(500).json(errResp(RC.SERVER_ERROR, "Server error"));
